...
@endif
    ...
    <div class="card-title" value=""><u><h3>{{$product['name']}}</h3></u></div><br/>
    ...
    <p>
    <div class="card-footer"><div class="text-left" style="color:gray"><b>Added On:</b>{{date('F d,Y',strtotime($product['created_at']))}} at {{date('d:ia',strtotime($product['created_at']))}}</div>

    <div class="text-right"><h3  style="color:darkblue"><b>रू.<u style="color:red">{{$product['price']}}</u>मात्र</b></h3></div></div>
    </p>
...
<div class="panel-heading py-2"><h3>Comments</h3></div>
                           ...
                           <!-- <p class="pull-left"><small>5 days ago</small></p> -->
                            ...
                            <small>{{$comment->created_at->diffForHumans()}}</small>
                             ...
                             <p><small> @if(\Auth::user()->id == $comment->user_id)
                              <a href="#"><form action="{{url('comments/'.$comment['id'])}}" method="post">
                                @csrf
                                <input type="hidden" name="_method" value="delete">
